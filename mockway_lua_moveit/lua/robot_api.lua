--[[
  ...
  在 Lua 脚本中 require 本文件即可使用：
      ...
      api.print_pose()
      api.jog_joint(1, 0.3, 1.0)   -- 关节1以0.3 rad/s点动1秒

  ── 全部可用 robot.* 接口速查 ──────────────────────────────────────────────
  【Servo 手动】
    robot.servo_joints({v1..v6})            -- 六轴速度 rad/s
    robot.servo_joint(name_or_idx, vel)     -- 单轴速度
  ...
  【MoveIt 规划执行】
    ...
    robot.move_to_joints({j1..j6})          -- 关节目标 PTP  (rad)
    robot.move_to_pose(x,y,z,qx,qy,qz,qw)  -- 位姿目标 PTP
    robot.move_to_pose_rpy(x,y,z,r,p,y)    -- RPY 版 PTP
    robot.move_linear(x,y,z,qx,qy,qz,qw)   -- 直线运动
    robot.move_linear_rpy(x,y,z,r,p,y)     -- RPY 版直线
    robot.move_linear_relative(dx,dy,dz,drx,dry,drz) -- 相对直线
  ...
  【状态查询】
    robot.get_joint_positions()   -> {j1..j6}
    robot.get_current_pose()      -> {x,y,z,qx,qy,qz,qw}
    robot.get_current_rpy()       -> {roll,pitch,yaw}

  【工具】
    robot.sleep(sec)
    robot.log(msg) / log_warn / log_error
    robot.ok()                    -> bool
    deg2rad(deg) / rad2deg(rad)   -- 全局函数
--]]
...
function M.print_joints()
  ...
  local s = string.format(
    ...
    rad2deg(j[1]), rad2deg(j[2]), rad2deg(j[3]),
    rad2deg(j[4]), rad2deg(j[5]), rad2deg(j[6]))
  robot.log(s)
end
...
function M.print_pose()
  ...
  robot.log(string.format(
    "末端位置: x=%.4f  y=%.4f  z=%.4f  |  RPY: %.1f°  %.1f°  %.1f°",
    p.x, p.y, p.z,
    rad2deg(r.roll), rad2deg(r.pitch), rad2deg(r.yaw)))
end

-- ── 关节点动一段时间后停止 ─────────────────────────────────────────────────
-- 参数: idx(1~6), velocity(rad/s), duration(s)
...
-- ── 笛卡尔点动一段时间后停止 ───────────────────────────────────────────────
-- 参数: vx,vy,vz,rx,ry,rz, duration(s) [, frame]
...
function M.joints_deg(a1, a2, a3, a4, a5, a6)
  return {deg2rad(a1), deg2rad(a2), deg2rad(a3),
          deg2rad(a4), deg2rad(a5), deg2rad(a6)}
end
